...
public class SaladItem extends Item {
	...
	public SaladItem(String name, FoodComponent foodComponent) {
	...
	public ItemStack finishUsing(ItemStack stack, World world, LivingEntity user) {
		if (!world.isClient && user instanceof PlayerEntity player) {
			RegistryEntry<StatusEffect> effect = CobbleCuisineEffects.NATURE_BUFF_MARKER.entry;
			if (player.hasStatusEffect(effect)) player.removeStatusEffect(effect);
			player.addStatusEffect(new StatusEffectInstance(effect, CobbleCuisineConfig.data.effectDuration.natureBoostEffectDuration, 0, false,false, false));
		}
