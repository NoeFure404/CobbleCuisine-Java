...
public class SandwichItem extends Item {
	...
	public ItemStack finishUsing(ItemStack stack, World world, LivingEntity user) {
		if (!world.isClient && user instanceof PlayerEntity player) {
			RegistryEntry<StatusEffect> effect = CobbleCuisineEffects.TERA_BUFF_MARKER.entry;
			if (player.hasStatusEffect(effect)) player.removeStatusEffect(effect);
			player.addStatusEffect(new StatusEffectInstance(effect, CobbleCuisineConfig.data.effectDuration.teraBoostEffectDuration, 0, false,false, false));
		}
